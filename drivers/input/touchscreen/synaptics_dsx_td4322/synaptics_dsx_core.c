...
static ssize_t cei_dynamic_log_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t cei_dynamic_log_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
#endif
...
#endif
#ifdef TP_DYNAMIC_LOG
	__ATTR(dynamic_log, (S_IRUGO | S_IWUSR | S_IWGRP),
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (retval < 0) {
		TP_LOGE("Failed to issue reset command, error = %d\n", retval);
		return retval;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (retval < 0) {
		TP_LOGE("Failed to read device status, error = %d\n", retval);
		return retval;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%02X%05X%08X\n",
			rmi4_data->tp_source,
			cooked_firmware_id,
			rmi4_data->config_id);
}
		...
		struct device_attribute *attr, char *buf)
{
	 ...
	 * tpfullfwver
	 *
	 *   SS_FFFFFFF_CCCCCCCC (19 chars)
	 *
	 *   SS (hex) = TP source
	 *   FFFFFFF (dec) = Firmware ID
	 *   CCCCCCCC (hex) = Config ID
	 */

	TP_LOGI("tpfullfwver = %02X_%u_0x%08X\n",
			rmi4_data->tp_source,
			rmi4_data->firmware_id,
			rmi4_data->config_id);

	return sprintf(buf, "%02X_%u_0x%08X\n",
			rmi4_data->tp_source,
			rmi4_data->firmware_id,
			rmi4_data->config_id);
}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			TP_LOGD("Finger %d: status = 0x%02x, x = %d, y = %d, wx = %d, wy = %d\n",
					finger,
					finger_status,
					x, y, wx, wy);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_PALM_STATUS:
			TP_LOGD("Finger %d: x = %d, y = %d, wx = %d, wy = %d\n",
					finger,
					x, y, wx, wy);
			break;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (retval < 0) {
		TP_LOGE("Failed to read button data registers\n");
		return;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	TP_LOGD("Function %02x reporting\n", fhandler->fn_number);
		...
		bool report)
{
	...
	if (retval < 0) {
		TP_LOGE("Failed to read interrupt status\n");
		return;
	}
	...
	if (status.status_code == STATUS_CRC_IN_PROGRESS) {
		retval = synaptics_rmi4_check_status(rmi4_data,
				&was_in_bl_mode);
		if (retval < 0) {
			TP_LOGE("Failed to check status\n");
			return;
		}
		retval = synaptics_rmi4_reg_read(rmi4_data,
				rmi4_data->f01_data_base_addr,
				...
				sizeof(status.data));
		if (retval < 0) {
			TP_LOGE("Failed to read device status\n");
			return;
		}
	}
	if (status.unconfigured && !status.flash_prog) {
		TP_LOGI("spontaneous reset detected\n");
		retval = synaptics_rmi4_reinit_device(rmi4_data);
		if (retval < 0) {
			TP_LOGW("Failed to reinit device\n");
		}
	}
		...
		bool enable, bool attn_only)
{
	...
	if (enable) {
		if (rmi4_data->irq_enabled) {
			TP_LOGI("Interrupt already enabled\n");
			goto exit;
		}
		...
		if (retval < 0) {
			TP_LOGE("Failed to create irq thread\n");
			goto exit;
		}
		...
		unsigned int intr_count)
{
	...
	if (!fhandler->extra) {
		TP_LOGE("Failed to alloc mem for fhandler->extra\n");
		return -ENOMEM;
	}
	extra_data = (struct synaptics_rmi4_f11_extra_data *)fhandler->extra;
	...
	TP_LOGD("Function %02x max x = %d max y = %d\n",
			fhandler->fn_number,
		...
		unsigned char sub)
{
	...
	if (reg > max_reg) {
		TP_LOGE("Register number (%d) over limit\n", reg);
		return -EINVAL;
	}
	...
	if ((presence[p_index] & (1 << bitnum)) == 0x00) {
		TP_LOGE("Register %d is not present\n", reg);
		return -EINVAL;
	}

	structure = kmalloc(presence[0], GFP_KERNEL);
	if (!structure) {
		TP_LOGE("Failed to alloc mem for structure register\n");
		return -ENOMEM;
	}
		...
		unsigned int intr_count)
{
	...
	if (!query_5) {
		TP_LOGE("Failed to alloc mem for query_5\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!query_8) {
		TP_LOGE("Failed to alloc mem for query_8\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!ctrl_8) {
		TP_LOGE("Failed to alloc mem for ctrl_8\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!ctrl_23) {
		TP_LOGE("Failed to alloc mem for ctrl_23\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!ctrl_31) {
		TP_LOGE("Failed to alloc mem for ctrl_31\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!ctrl_58) {
		TP_LOGE("Failed to alloc mem for ctrl_58\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!fhandler->data) {
		TP_LOGE("Failed to alloc mem for fhandler->data\n");
		retval = -ENOMEM;
		goto exit;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!f1a) {
		TP_LOGE("Failed to alloc mem for function handle\n");
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to read query registers\n");
		return retval;
	}
	...
	if (!f1a->button_control.txrx_map) {
		TP_LOGE("Failed to alloc mem for tx rx mapping\n");
		return -ENOMEM;
	}
	...
	if (!f1a->button_map) {
		TP_LOGE("Failed to alloc mem for button map\n");
		return -ENOMEM;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (f1a->button_query.has_tx_rx_map) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				f1a->max_count * 2);
		if (retval < 0) {
			TP_LOGE("Failed to read tx rx mapping\n");
			return retval;
		}
	...
	if (f1a->button_query.has_query4) {
		...
		retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				sizeof(query_4.data));
		if (retval < 0) {
			TP_LOGE("Failed to read button features 4\n");
			return retval;
		}
	...
	if (!bdata->cap_button_map) {
		TP_LOGE("cap_button_map is NULL in board file\n");
		return -ENODEV;
	} else if (!bdata->cap_button_map->map) {
		TP_LOGE("Button map is missing in board file\n");
		return -ENODEV;
	} else {
		...
		bool *was_in_bl_mode)
{
	...
	if (status.flash_prog == 1) {
		...
		TP_LOGI("In flash prog mode, status = 0x%02x\n",
				status.status_code);
	} else {
...
static void synaptics_rmi4_set_configured(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		TP_LOGE("Failed to set configured\n");
		return;
	}
...
rescan_pdt:
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			if (rmi_fd.fn_number == 0) {
				TP_LOGD("Reached end of PDT\n");
				break;
			}

			TP_LOGD("F%02x found (page %d)\n",
					rmi_fd.fn_number,
					page_number);
			...
			case SYNAPTICS_RMI4_F01:
				...
				retval = synaptics_rmi4_alloc_fh(&fhandler,
						&rmi_fd, page_number);
				if (retval < 0) {
					TP_LOGE("Failed to alloc for F%d\n",
							rmi_fd.fn_number);
					return retval;
				}
	...
	if (!f01found) {
		TP_LOGW("Failed to find F01\n");

		if (!f35found) {
			TP_LOGE("Failed to find F35\n");
			return -EINVAL;
		} else {
			TP_LOGI("In microbootloader mode\n");
			return 0;
		}
	}
...
flash_prog_mode:
	...
	TP_LOGD("Number of interrupt registers = %d\n",
			rmi4_data->num_of_intr_regs);

	f01_query = kmalloc(F01_STD_QUERY_LEN, GFP_KERNEL);
	if (!f01_query) {
		TP_LOGE("Failed to alloc mem for f01_query\n");
		return -ENOMEM;
	}
	...
	retval = secure_memcpy(rmi->product_id_string,
			...
			PRODUCT_ID_SIZE);
	if (retval < 0) {
		TP_LOGE("Failed to copy product ID string\n");
	}

	kfree(f01_query);

	if (rmi->manufacturer_id != 1) {
		TP_LOGE("Non-Synaptics device found, manufacturer ID = %d\n",
				rmi->manufacturer_id);
	}
...
static int synaptics_rmi4_gpio_setup(int gpio, bool config, int dir, int state)
{
	...
	if (config) {
		...
		if (retval) {
			TP_LOGE("Failed to get gpio %d (code: %d)",
					gpio, retval);
			return retval;
		}
		...
		if (retval) {
			TP_LOGE("Failed to set gpio %d direction",
					gpio);
			return retval;
		}
	} else {
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		TP_LOGE("Failed to query device\n");
	...
	if (retval) {
		TP_LOGE("Failed to register input device\n");
	...
	if (rmi4_data->stylus_dev == NULL) {
		TP_LOGE("Failed to allocate stylus device\n");
		retval = -ENOMEM;
	...
	if (retval) {
		TP_LOGE("Failed to register stylus device\n");
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_gpio_setup(
			...
			true, 0, 0);
	if (retval < 0) {
		TP_LOGE("Failed to configure attention GPIO\n");
		goto err_gpio_irq;
	}
	...
	if (bdata->power_gpio >= 0) {
		...
		if (retval < 0) {
			TP_LOGE("Failed to configure power GPIO\n");
			goto err_gpio_power;
		}
	}
	...
	if (bdata->reset_gpio >= 0) {
		...
		if (retval < 0) {
			TP_LOGE("Failed to configure reset GPIO\n");
			goto err_gpio_reset;
		}
	}
...
static int synaptics_rmi4_pinctrl_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (IS_ERR_OR_NULL(rmi4_data->ts_pinctrl)) {
		...
		TP_LOGE("Target does not use pinctrl %d\n", retval);
	...
	if (IS_ERR_OR_NULL(rmi4_data->pinctrl_state_active)) {
		...
		TP_LOGE("Can not lookup %s pinstate %d\n",
			"pmx_ts_active", retval);
		...
		bool get)
{
	...
	if ((bdata->pwr_reg_name != NULL) && (*bdata->pwr_reg_name != 0)) {
		...
		if (IS_ERR(rmi4_data->pwr_reg)) {
			TP_LOGE("Failed to get power regulator\n");
	...
	if ((bdata->bus_reg_name != NULL) && (*bdata->bus_reg_name != 0)) {
		...
		if (IS_ERR(rmi4_data->bus_reg)) {
			TP_LOGE("Failed to get bus pullup regulator\n");
		...
		bool enable)
{
	...
	if (rmi4_data->bus_reg) {
		...
		if (retval < 0) {
			TP_LOGE("Failed to enable bus pullup regulator\n");
			goto exit;
		}
	}
	...
	if (rmi4_data->pwr_reg) {
		...
		if (retval < 0) {
			TP_LOGE("Failed to enable power regulator\n");
...
static void synaptics_rmi4_rebuild_work(struct work_struct *work)
{
	...
	if (retval < 0) {
		TP_LOGE("Failed to set up input device\n");
		goto exit;
	}
		...
		bool enable)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		TP_LOGE("Failed to read device control\n");
		return;
	}
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		TP_LOGE("Failed to write device control\n");
		return;
	}
		...
		bool insert)
{
	...
	mutex_lock(&exp_data.mutex);
	if (insert) {
		TP_LOGI("synaptics_rmi4_new_function true\n");

		exp_fhandler = kzalloc(sizeof(*exp_fhandler), GFP_KERNEL);
		if (!exp_fhandler) {
			TP_LOGE("Failed to alloc mem for expansion function\n");
			goto exit;
		}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if (!hw_if) {
		TP_LOGE("No hardware interface found\n");
		fw_is_update = false;
		return -EINVAL;
	}
	...
	if (!rmi4_data) {
		TP_LOGE("Failed to alloc mem for rmi4_data\n");
		fw_is_update = false;
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to get regulators\n");
		goto err_get_reg;
	}

	retval = synaptics_rmi4_enable_reg(rmi4_data, true);
	if (retval < 0) {
		TP_LOGE("Failed to enable regulators\n");
		goto err_enable_reg;
	}
	...
	if (!retval && rmi4_data->ts_pinctrl) {
		retval = pinctrl_select_state(rmi4_data->ts_pinctrl,
			rmi4_data->pinctrl_state_active);
		if (retval < 0) {
			TP_LOGE("Failed to select %s pinstate %d\n",
				"pmx_ts_active", retval);
		}
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to set up GPIO's\n");
		goto err_set_gpio;
	}
	...
	if (hw_if->ui_hw_init) {
		...
		if (retval < 0) {
			TP_LOGE("Failed to initialize hardware interface\n");
			goto err_ui_hw_init;
		}
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to enable attention interrupt\n");
		goto err_enable_irq;
	}
	...
	rmi4_data->touchscreen_link = kobject_create_and_add("touchscreen", NULL);
		...
		bool enable)
{
	...
	if (retval < 0) {
		TP_LOGE("Failed to change reporting mode\n");
		return;
	}
...
exit:
	...
	if (rmi4_data->suspend) {
		retval = synaptics_rmi4_reset_device(rmi4_data, false);
		if (retval < 0) {
			TP_LOGW("Failed to issue reset command\n");
		}
	}
#endif
	mutex_lock(&exp_data.mutex);
